from inventory_app.services.category_service import create_category, get_all_categories, delete_category
from inventory_app.services.supplier_service import create_supplier, get_all_suppliers, delete_supplier
...
def test_category_service(app, mock_mongo):
    with app.app_context():
        # Create category
        success, msg, cat = create_category("Cement", "All cement types", "admin")
        assert success
        assert cat["name"] == "Cement"

        # Duplicate category
        success2, msg2, _ = create_category("Cement", "Duplicate", "admin")
        assert not success2
        ...
        # Delete category (succeeds when no products attached)
        del_ok, del_msg = delete_category(cat["_id"], "admin")
        assert del_ok

def test_supplier_service(app, mock_mongo):
    with app.app_context():
        # Create supplier
        success, msg, sup = create_supplier("SUP01", "JK Cement Ltd", "Rajesh", "9876543210", "jk@supplier.com", "admin")
        assert success
        assert sup["code"] == "SUP01"

        # Duplicate code check
        dup_ok, dup_msg, _ = create_supplier("SUP01", "Other", "Rajesh", "9876543210", "jk@supplier.com", "admin")
        assert not dup_ok
        ...
        # Delete supplier
        del_ok, del_msg = delete_supplier(sup["_id"], "admin")
        assert del_ok
...
def test_routes_endpoints(admin_client):
    ...
    res = admin_client.get('/categories')
    assert res.status_code == 200

    res = admin_client.post('/categories/add', data={'name': 'Plumbing', 'description': 'Pipes & fittings'}, follow_redirects=True)
    assert res.status_code == 200

    # Test suppliers routes
    res = admin_client.get('/suppliers')
    assert res.status_code == 200

    res = admin_client.post('/suppliers/add', data={'code': 'SUP02', 'name': 'UltraTech Supplier'}, follow_redirects=True)
    assert res.status_code == 200
