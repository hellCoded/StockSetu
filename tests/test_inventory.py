...
def test_status_badge_transitions(manager_client, staff_client):
    ...
    # Initial status: IN STOCK (20 > 10)
    res1 = staff_client.get('/products/Status Widget', follow_redirects=True)
    assert b"IN STOCK" in res1.data
    
    # Stock out 15 -> quantity becomes 5 (5 <= 10 -> LOW STOCK)
    ...
    assert b"LOW STOCK" in res2.data
    
    # Stock out remaining 5 -> quantity becomes 0 (OUT OF STOCK)
    staff_client.post('/inventory/stock-out', data={
        'product_name': 'Status Widget',
        'quantity': '5',
    ...
    assert b"OUT OF STOCK" in res3.data
